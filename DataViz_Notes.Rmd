...
## CH2 Get Started
Everything has a name in R
Everything is an object in R
Shortcut for the assign operator: Alt+minus
You do things in R through functions
Functions come in libraries
To find out what the object is, ask for its class [class()]
...
## CH3 Make a Plot
In ggplot, first tell it what is the data
In ggplot, the connection between data and plot elements is called aesthetic mappings or just aesthetic.
In ggplot, the overall type of plot is called geom, defining how we want to see the data
Layer on geoms as needed
Use additional functions to adjust scales, labels, tick marks, titles
...
p <- ggplot(data = gapminder, 
            mapping = aes(x=gdpPercap, 
                          y=lifeExp))
p_out <- p + geom_point(color = "black",
                        alpha = 0.3) +
...
ggsave(here("Figures", "f1_lifexp_vs_gdp.pdf"), 
       plot = p_out, height = 8, width = 10, units = "cm")

```


## CH4 Show the Right Numbers

The group aesthetic is usually only needed when the grouping information you need to tell ggplot about is not built-in to the variables being mapped. When mapping the x to year, however, there is no information in the year variable itself to let ggplot know that it is grouped by country for the purposes of drawing lines with it. So we need to say that explicitly.
...
## CH5 Graph Tables, Make Labels, Add Notes
